...
class ResponseProtocolCompliance {
    ...
    private void ensure200ForOPTIONSRequestWithNoBodyHasContentLengthZero(HttpRequest request, HttpResponse response) {
        if (request.getRequestLine().getMethod().equalsIgnoreCase("OPTIONS") && response.getStatusLine().getStatusCode() == HttpStatus.SC_OK && response.getFirstHeader("Content-Length") == null) {
