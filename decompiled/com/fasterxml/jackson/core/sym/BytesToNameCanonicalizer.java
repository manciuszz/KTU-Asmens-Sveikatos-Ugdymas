...
public final class BytesToNameCanonicalizer {
    ...
    private void mergeChild(TableInfo tableInfo) {
        ...
        if (i > tableInfo2.count) {
            if (i > 6000 || tableInfo.longestCollisionList > MAX_COLL_CHAIN_FOR_REUSE) {
    ...
    private void unshareCollision() {
        Object[] objArr = this._collList;
        if (objArr == null) {
            this._collList = new Bucket[32];
        } else {
            this._collList = (Bucket[]) ArraysCompat.copyOf(objArr, objArr.length);
        }
        this._collListShared = false;
    }

    private void unshareNames() {
        Object[] objArr = this._mainNames;
        this._mainNames = (Name[]) ArraysCompat.copyOf(objArr, objArr.length);
        this._mainNamesShared = false;
    }

    private void expandCollision() {
        Object[] objArr = this._collList;
        this._collList = (Bucket[]) ArraysCompat.copyOf(objArr, objArr.length * 2);
    }
